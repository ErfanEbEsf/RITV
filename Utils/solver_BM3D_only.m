...
function out = solver_BM3D_only (maxit,b,mask,x0,param)
...
for mm = 1:maxit
    ...
    KTy_u =  R_adj(r); 
    u = u - tau*(KTy_u);
    [~,u] = BM3D_MRI_denoise2(eta*tau,1,u,sigma_bm3d(mm)); 
    %v_old = v;
    %KTy_v = L(h);
    ...
    if sqrt(beta)*tau*normX(R_adj(r) ...
            -KTy_u,0) <= delta*normY(0,r-r_old)
      break;
    else
        tau = tau*mu;
    end
